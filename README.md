...
| 功能 | 说明 |
|------|------|
| CSV 上传分析 | 拖入或点击上传 CSV → **浏览器本地解析**（无大小限制）→ 可搜索的下拉多选框选择类别 → 添加到下载列表 |
| 批量下载 | 指定类别名和数量上限；提交前自动过滤 CSV 只发送匹配行（解决代理 body size 限制问题） |
...
| 功能 | 说明 |
|------|------|
| 单数据集模式 | 下拉选择数据集 + 填写 Caption，适合单一来源 |
| **多数据集模式** | 切换后可添加多个数据集行，**每行独立填写 Caption**，数据集管理里点「→训练」自动追加一行 |
| 验证 Prompt | 留空时自动用 `trigger + caption` 拼接（多数据集用第一个来源） |
| 续训 / 重跑 | 输入历史任务 ID 点「查询」，自动判断：有 checkpoint 则续训；仅有预处理数据则跳过预处理重跑；多数据集任务自动切换到多数据集模式并填好各行 Caption |
